...
class Morders extends CI_Model {
    ...
    function ordersToComplete() {
        $Q = $this->db->get_where('order', array('delivery_date' => 0));
        return $Q;
    }

    function getOrderDetails($id) {
        $this->db->select('order_item.order_item_id,order_item.order_id,order_item.product_id,
						   order_item.quantity,order_item.price,product.name,
						   order.order_date, order.delivery_date, order.payment_date');
    ...
    function exportProduct() {
        $file_now = 'order_' . date('Ymd-H_i_s') . '.csv';
        $query = $this->db->query(" SELECT 'Order Id','First Name','Last Name','Email','Total','Telephone','Order Date',
                                    'Address','City','State','Zip Code'
                                    UNION SELECT
                                    o.order_id , customer_first_name,customer_last_name,email,total , phone_number,order_date,

...
class Products extends Admin_Controller {
            ...
            foreach ($this->options['image_versions'] as $version => $options) {
                if (is_file($options['upload_dir'] . $file_name)) {
                    $file->{$version . '_url'} = $options['upload_url']
                            . rawurlencode(del_ext($file->name));
                }
            }
    ...
    private function handle_file_upload($uploaded_file, $name, $size, $type, $error) {
        ...
        if (!$error && $file->name) {
            ...
            $file->image_id = del_ext($file->name);
            $file->size = $file_size;
            $file->delete_url = site_url('/products/delete_upload/')
                    . '/' . rawurlencode(del_ext($file->name));
            $file->delete_type = 'DELETE';
    ...
    public function getImg($id) {
        ...
        if ($data_img) {
            foreach ($data_img as $row_img) {
                $row_img->delete_url = site_url('/products/delete_upload/') . '/' . rawurlencode($row_img->image_id);
                ...
                $row_img->default = $row_img->default == 1 ? 'Primary' : '';
            }
        } else {
            $data_img = array();
            //$data_img = null;
    ...
    public function change_primary($id) {
        $this->mproducts->changePrimary($id);
        $output = 'beres';
        header('Content-type: application/json');
    ...
    public function delete_test() {
        unlink(basic_path() . 'assets/product/22.gif');
        unlink(basic_path() . 'assets/product_thumb/33.gif');
        echo del_ext('23.gif');
    }

    public function delete_upload($id) {
        $img = $this->mproducts->getImg($id);
        $this->mproducts->deleteImg($id);
        $file_name = isset($img->name) ? basename(stripslashes($img->name)) : null;
        $file_path = basic_path() . $img->url . $img->image_id . $img->extension;
        $file_path_thumb = basic_path() . $img->thumbnail_url . $img->image_id . $img->extension;
        echo $file_path_thumb;
